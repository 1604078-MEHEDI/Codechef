...
#define maxn 1000010 // maximal size of the string
#define inf 1000000000 // *infinity*
#define mod 1000000007 // modulo from the statement
...
int n,i,N,sz,qL,qR,que[maxn],endless,idx,j,leng;
char ch,s[maxn];
...
int main(){	
	scanf("%d\n",&n); // n is the number of queries
	assert(1<=n&&n<=1000000);
	sz=1,qL=1,qR=0;ptr.v=1;t[1].link=1; // create an empty suffix tree
	for(i=1;i<=n;i++){
		ch=getchar(); // get the operation type
		while(ch!='+'&&ch!='-')ch=getchar();
		if(ch=='+'){ // add the symbol
			getchar();
			ch=getchar();
			assert('a'<=ch&&ch<='z');
			++leng;
